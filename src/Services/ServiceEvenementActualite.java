...
import utils.MyDb;
import utils.SendWhatsAppMessage;

import java.sql.*;
import java.util.ArrayList;
import java.util.List;

public class ServiceEvenementActualite implements IServiceActualite<Evenement> {
    ...
    public void ajouter(Evenement evenement) {
        try {
            ...
            SendWhatsAppMessage.sendMessage("whatsapp:+21628546430");
            SendWhatsAppMessage.sendMessage("whatsapp:+21623483895");
    ...
    public  List<Evenement> rech (String nom) {
        List<Evenement> evenements = new ArrayList();
        boolean test ;
        try {
            String querry="SELECT * FROM evenement WHERE `nom` like '"+nom+"%'";
            Statement stm =cnx.createStatement();
            ResultSet rs=stm.executeQuery(querry);
            ...
            /*test = evenements.stream().anyMatch((p -> p.getNom()==nom ));
            if (test ==true ){
